...
if __name__ == '__main__':
    ...
    for file_index in tqdm(thumbnail_paths.keys()):
        ...
        mask = np.where((mask - 255.) * -1 > 5, 255, 0)
        mask = mask.astype(np.uint8)
        mask = cv2.cvtColor(mask, cv2.COLOR_GRAY2BGR)

        if thumbnail.shape[:2] != mask.shape[:2]:
            print(thumbnail.shape, mask.shape)
            raise AssertionError

        temp_thumbnail = np.ones_like(thumbnail) * 255.
        thumbnail = np.where(mask > 127, thumbnail, temp_thumbnail)
    ...
    cv2.imwrite(os.path.join(save_dir, 'target_5.png'), output)
